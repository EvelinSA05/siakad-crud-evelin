...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-700 p-8 max-w-3xl mx-auto transition-all duration-300">
    <div class="mb-8">
        <a href="{{ route('mata-kuliah.index') }}" class="inline-flex items-center text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 hover:underline text-sm font-medium transition-colors mb-4">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            Kembali ke daftar
        </a>
        <h1 class="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Tambah Mata Kuliah</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-2">Lengkapi formulir di bawah untuk menambahkan data mata kuliah baru.</p>
    </div>
    ...
    <form action="{{ route('mata-kuliah.store') }}" method="POST" class="space-y-6">
        ...
        <div class="flex items-center gap-4 pt-6 border-t border-slate-100 dark:border-slate-700 transition-colors">
            ...
            <a href="{{ route('mata-kuliah.index') }}" class="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600 font-medium px-8 py-2.5 rounded-lg shadow-sm transition-all duration-200">
                Batal
            </a>
        </div>
    </form>
